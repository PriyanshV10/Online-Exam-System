package com.exam.util;

import java.io.IOException;

import com.exam.model.ApiResponse;

import jakarta.servlet.http.HttpServletResponse;

public class ResponseUtil {

    private ResponseUtil() {}

    private static void write(
            HttpServletResponse response,
            int status,
            ApiResponse<?> body) throws IOException {

        response.setStatus(status);
        response.setContentType("application/json;charset=UTF-8");

        if (body != null) {
            response.getWriter().write(JsonUtil.toJson(body));
        }
    }

    /* ---------- Success ---------- */

    public static void ok(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_OK, body);
    }

    public static void created(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_CREATED, body);
    }

    public static void noContent(HttpServletResponse response) {
        response.setStatus(HttpServletResponse.SC_NO_CONTENT);
    }

    /* ---------- Client Errors ---------- */

    public static void badRequest(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_BAD_REQUEST, body);
    }
    
    public static void unauthorized(HttpServletResponse response) throws IOException {
    	unauthorized(response, ApiResponse.error("Authentication required"));
    }

    public static void unauthorized(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_UNAUTHORIZED, body);
    }

    public static void forbidden(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_FORBIDDEN, body);
    }

    public static void notFound(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_NOT_FOUND, body);
    }

    public static void conflict(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_CONFLICT, body);
    }

    /* ---------- Server Error ---------- */

    public static void serverError(HttpServletResponse response, ApiResponse<?> body)
            throws IOException {
        write(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, body);
    }
}
